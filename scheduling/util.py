...
def get_or_variable(csp, name, variables, value):
    ...
    for i, X_i in enumerate(variables):
        ...
        csp.add_variable(B_i, ['prev', 'equals', 'no'])
        ...
        if i == 0:
            ...
            csp.add_unary_factor(B_i, lambda b: b != 'prev')
        else:
            # consistency between B_{i-1} and B_i
            def factor(b1, b2):
                if b1 in ['equals', 'prev']: return b2 != 'no'
                return b2 != 'prev'
            csp.add_binary_factor(('or', name, i - 1), B_i, factor)

    # consistency between B_n and result
    # hacky: reuse B_i because of python's loose scope
    csp.add_binary_factor(B_i, result, lambda val, res: res == (val != 'no'))
    return result
