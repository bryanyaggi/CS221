...
def test4a_1():
    random.seed(3)
    pf = submission.ParticleFilter(30, 13)
    grader.requireIsEqual(69, len([k for k,v in pf.particles.iteritems() if v > 0])) # This should not fail unless your code changed the random initialization code.

    pf.elapseTime()
    grader.requireIsEqual(200, sum(pf.particles.values())) # Do not lose particles
    grader.requireIsEqual(64, len([k for k,v in pf.particles.iteritems() if v > 0])) # Most particles lie on the same (row, col) locations
    ...
    pf.elapseTime()
    grader.requireIsEqual(200, sum(pf.particles.values())) # Do not lose particles
    grader.requireIsEqual(63, len([k for k,v in pf.particles.iteritems() if v > 0])) # Slightly more particles lie on the same (row, col) locations
...
def test4a_2():
    ...
    grader.requireIsEqual(200, sum(pf.particles.values())) # Do not lose particles
    grader.requireIsEqual(3, len([k for k,v in pf.particles.iteritems() if v > 0])) # Most particles lie on the same (row, col) locations
    ...
    pf.elapseTime()
    grader.requireIsEqual(6, len([k for k,v in pf.particles.iteritems() if v > 0])) # Most particles lie on the same (row, col) locations
